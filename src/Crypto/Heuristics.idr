...
scoreText str =
  ...
  where
    ...
    score : Integer -> (Char, Nat) -> Integer
    score acc (char, rank) =
      let target = letterRank idealRanks char
          gtr = toIntegerNat $ maximum target rank
          smlr = toIntegerNat $ minimum target rank
      in
        acc - gtr - smlr

scoreSorter : String -> String -> Ordering
scoreSorter x y = (scoreText x) `compare` (scoreText y)
